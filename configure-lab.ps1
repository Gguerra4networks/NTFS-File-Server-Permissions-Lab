...
function Invoke-VMScript {
  ...
  # Use .Replace() for literal substitution. -replace treats BOTH sides as regex, so
  # [regex]::Escape() on the replacement side doesn't protect it, it inserts a literal
  # backslash before special characters (e.g. "Pass$123" becomes "Pass\$123"), corrupting
  # any password containing $, and causing domain join to fail with "user name or
  # password is incorrect" even though the password itself was never actually wrong.
  foreach ($key in $Replacements.Keys) { $script = $script.Replace($key, $Replacements[$key]) }
...
try { Invoke-VMScript -VMName "DC01" -ScriptPath ".\scripts\00-promote-dc.ps1" -Description "Promoting DC01" `
